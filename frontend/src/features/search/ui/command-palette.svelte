<script>
	import { hideSearch, isSearchVisible } from '../model';
	import { Link } from 'svelte-navigator';

	$: backgroundOverlayTransitionClasses = $isSearchVisible
		? 'ease-in duration-200 opacity-100'
		: 'ease-out duration-300 opacity-0';
	$: commandPaletteTransitionClasses = $isSearchVisible
		? 'ease-in duration-200 opacity-100 scale-100'
...
{#if $isSearchVisible}
	...
	>
		<div
			...
			class="{backgroundOverlayTransitionClasses} fixed inset-0 bg-gray-500 bg-opacity-25 transition-opacity"
			aria-hidden="true"
		/>
		<div
			class="{commandPaletteTransitionClasses} mx-auto max-w-3xl transform divide-y divide-gray-100 overflow-hidden rounded-xl bg-white shadow-2xl ring-1 ring-black ring-opacity-5 transition-all"
		>
			<div class="relative">
			...
			<div class="grid sm:grid-cols-2 divide-x divide-gray-100">
				<!-- Preview Visible: "sm:h-96" -->
				<div class="max-h-1/2 min-w-0 flex-auto scroll-py-4 overflow-y-auto px-6 py-4 sm:h-96">
					...
					<ul class="-mx-2 text-sm text-gray-700 overflow-y-auto" id="options" role="listbox">
						<!-- Active: "bg-gray-100 text-gray-900" -->
						<li
							class="group flex cursor-default select-none items-center rounded-md p-2 "
							id="option-1"
				...
				<div class="sm:border-t max-h-1/2 flex-col divide-y divide-gray-100 overflow-y-auto flex">
					...
					<div class="flex flex-auto flex-col justify-between p-6">
						<dl class="grid grid-cols-1 gap-x-6 gap-y-3 text-sm text-gray-700">
							...
							<dd class="truncate">
								<Link to="https://example.com" class="text-emerald-600 underline"
									>https://example.com</Link
								>
							</dd>
							<dt class="col-end-1 font-semibold text-gray-900">Email</dt>
							<dd class="truncate">
								<Link to="#" class="text-emerald-600 underline">tomcook@example.com</Link>
							</dd>
						</dl>
						<button
							type="button"
							class="mt-6 w-full rounded-md border border-transparent bg-emerald-600 py-2 px-4 text-sm font-medium text-white shadow-sm hover:bg-emerald-700 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2"
							>Send message</button
						>
					</div>
				</div>
